...
public class Sha4J {
	...
	public byte[] sha1Digest(InputStream inputStream) throws IOException {
		...
		for (;;) {
			...
			int a = h[0];// ref : 1732584193;
			int b = h[1];// ref : 4023233417;
			int c = h[2];// ref : 2562383102;
			int d = h[3];// ref : 271733878;
			int e = h[4];// ref : 3285377520;

			int t = 0;
			for (; t < 80; t++) {
				int T = modulo32Add(
						modulo32Add(modulo32Add(ROTL(5, a), f(t, b, c, d)),
			...
			// ref : interMediateHash128(H,a,b,c,d,e);
	...
	private int f(int t, int x, int y, int z) {
		if (t < 20) {
			return (x & y) ^ (~x & z);// ref :Ch(x, y, z);
		}
		if (t < 40) {
			return x ^ y ^ z;// ref : Parity(x, y, z);
		}
		if (t < 60) {
			return (x & y) ^ (x & z) ^ (y & z);// ref : Maj(x, y, z);
		}
   ...
   private byte[] sha256Digest(InputStream  inputStream,boolean sha224) throws IOException {
     ...
     int[] hs =  { h0, h1, h2, h3, h4, h5, h6, h7 };
     ...
     for (;;)
     {
       if (bytesRead == -1){
         ...
         int j = 0;
         for (; j < 8; i += 4, j++){
           digest[i] = (byte) (hs[j] >>> 24);
           digest[i + 1] = (byte) (hs[j] >>> 16);
           digest[i + 2] = (byte) (hs[j] >>> 8);
           digest[i + 3] = (byte) (hs[j]);
         } 
         if(closeInputStream){
           inputStream.close();
         }
       ...
       if (bytesRead > 55 && bytesRead < 64){
         ...
         padBlock = true;// padBlock is 448-bits of zero + 64 bit length
			...
			// init working variables
			int a = hs[0];// ref : 1732584193;
			int b = hs[1];// ref : 4023233417;
			int c = hs[2];// ref : 2562383102;
			int d = hs[3];// ref : 271733878;
			int e = hs[4];// ref : 3285377520;
			int f = hs[5];// ref : 3285377520;
			int g = hs[6];// ref : 3285377520;
			int h = hs[7];// ref : 3285377520;

			int t = 0;
			for (; t < 64; t++) {
				int T1 = modulo32Add(
						modulo32Add(modulo32Add(h, SIGMA256_1(e)),
			...
			hs[0] = modulo32Add(hs[0], a);
			hs[1] = modulo32Add(hs[1], b);
			hs[2] = modulo32Add(hs[2], c);
			hs[3] = modulo32Add(hs[3], d);
			hs[4] = modulo32Add(hs[4], e);
			hs[5] = modulo32Add(hs[5], f);
			hs[6] = modulo32Add(hs[6], g);
			hs[7] = modulo32Add(hs[7], h);

			// do last pad
			if (padBlock) {
				doPadBlock256(hs, m, w, false);
			}
	...
	private byte[] sha512Digest(InputStream inputStream, boolean sha384)
		...
		for (;;) {
			...
			int t = 0;
			for (; t < 80; t++) {
				long T1 = modulo64Add(modulo64Add(h, SIGMA512_1(e)),
						modulo64Add(modulo64Add(Ch(e, f, g), K512[t]), W[t]));
	...
	private void doPadBlock512(long[] H, long[] M, long[] W, boolean addOne) {
		...
		// init working variables
		long a = H[0];// ref : 1732584193;
		long b = H[1];// ref : 4023233417;
		long c = H[2];// ref : 2562383102;
		long d = H[3];// ref : 271733878;
		long e = H[4];// ref : 3285377520;
		long f = H[5];// ref : 3285377520;
		long g = H[6];// ref : 3285377520;
		long h = H[7];// ref : 3285377520;
		...
		for (; t < 80; t++) {
			T1 = modulo64Add(
					modulo64Add(
							modulo64Add(modulo64Add(h, SIGMA512_1(e)),
									Ch(e, f, g)), K512[t]), W[t]);
